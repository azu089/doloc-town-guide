...
(function(){
  ...
  function loadAd(){if(!cfg.adsterra||adLoaded||document.getElementById('doloc-adsterra-script'))return;if(!ensureAdContainer())return;var note=document.querySelector('.native-ad-consent-note');if(note){var notes=document.querySelectorAll('.native-ad-consent-note');for(var ni=0;ni<notes.length;ni++)notes[ni].remove();}var s=document.createElement('script');s.id='doloc-adsterra-script';s.async=true;s.setAttribute('data-cfasync','false');s.src=cfg.adsterra.src;document.body.appendChild(s);adLoaded=true;}
